...
1. 純助詞 run の一致: 単体（`は` `が` `を` `に` `で` `と` `も` `へ` `の` `や` `か` `ね` `よ`
   `な` `わ` `け` `ば` `ぞ` `ぜ`）、疑問・並立・接続（`かな` `かしら` `やら` `なり` `つつ`
   `ながら` `ものの`）、程度（`ほど` `くらい` `ぐらい` `ごろ` `ころ` `さえ` `すら` `だけ`
   `しか` `こそ`）、複合（`から` `まで` `より` `って` `には` `では` `への` `との` `での` `ので`
   `のに` `のは` `のが` `のを` `のも` `にも` `でも` `とも` `とは` `など` `なんて` `だの` `とか`
   `けど` `のみ` `ばかり` `けれど` `けれども` `について` `にとって` `に対して` `によって`
   `により` `に関して` `を通じて` `において`）のどれかと一致する kana run が
   削除側・挿入側に現れた場合。単体の「さ」は語の一部（くだい→ください 等）と
   区別できないため集合に入れない（既知の限界）
2. 両側が kana のみの変更で、片側が 2 code points 以上の組
   （例: 「さえ」→「すら」、「がい」→「をみ」、「ほど」→「くらい」）。
   助詞と語句の同時置換を含む kana 変更を包括的に拒否する。
   助詞でないかな語の修正は 1 文字交換（例: 「しますた」→「しました」）の
   形をとるため許可される（docs の末尾「既知の限界」参照）
...
- 環境: Linux x86_64 / Node.js v26.8.1（node:test、type stripping）
- gate CLI: 基準 commit `dac0954` の固定 binary（digest `fa3436f9…`、docs/compat/jp-quality-gate-cli.md）
- policyDigest: `${PINNED_GATE_SHA256}/${TECH_MINIMAL_PROFILE_VERSION}`（pre/post 比較時に一致を必須化）
- 試験: `node --test test/japanese/{sentinel,structural,semantic-risk,adoption,pipeline}.test.ts`
  （公開イベント試験は pipeline.test.ts、実行日時は各 commit の記録）
- 再検証条件: 助詞集合・marker 一覧・閾値・protected pattern・envelope 契約の
  いずれかを変更したとき、全 fixture を再実行して本記録の表を更新する
...
- RISK_TOKENS は部分一致のため、token を含む一般語の修正を拒否することがある
  （fail-closed の方向で許容）
- 助詞変更の検査は2段で構成する: (1) 助詞集合との完全一致、(2) 両側が
  kana のみの変更で片側 2 code points 以上（例: 「さえ」→「すら」「がい」→「をみ」）。
  助詞以外のかな語の修正は 1 文字交換（例: 「しますた」→「しました」）の形を
  とるため許可される。かな語の 2 文字以上の削除・挿入（例: 重複かなの除去）も
  この検査で拒否される（fail-closed）。したがって、1 文字 kana 交換が
  未登録の助詞変更と同じ形をとる場合は区別できない（kana と助詞の判定には
  形態素解析が要るため、Issue #9 の許可助詞パターン導入までこの限界を許容）
